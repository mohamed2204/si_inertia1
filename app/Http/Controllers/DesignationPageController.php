...
class DesignationPageController extends Controller
{
    ...
    public function edit($id)
    {
        ...
        foreach ($designation->items as $item) {
            // Extraire le nom du jour en français (ex: "vendredi")
            $jourKey = Carbon::parse($item->date_effective)->translatedFormat('l');
            $jourKey = ucfirst($jourKey); // Devient "Vendredi" pour s'aligner avec le front

            // Structuration : [lab_id][nom_du_jour][requis_id] = membre_id
            $formattedItems[$item->laboratoire_id][$jourKey][$item->laboratoire_config_id] = $item->membre_id;
        }

        // 🔥 ASTUCE FINALE : On convertit le modèle en tableau et on y injecte de force les items formatés
        $designationArray = $designation->toArray();
        $designationArray['formatted_items'] = empty($formattedItems) ? (object) [] : $formattedItems;

        //dd($designationArray); // Debug : vérifier la structure finale envoyée au Front

        return Inertia::render('Designations/FormDesignation', [
            'departements' => Departement::all(['id', 'nom']),
            'designation'  => $designationArray, // 👈 On envoie le tableau enrichi à Inertia
        ]);
    }
    // public function edit($id)
    // {
    ...
    {
        // Forcer le statut initial si absent
        if (!$request->has('statut')) {
            $request->merge(['statut' => 'en_attente']);
        }

        // Exécuter le traitement partagé (renvoie une instance de Designation ou une redirection/erreur)
        $designation = $this->enregistrerOuMettreAJour($request);
    ...
    {
        // Conserver le statut actuel si absent
        if (!$request->has('statut')) {
            $request->merge(['statut' => $designation->statut ?? 'en_attente']);
        }

        // Exécuter le traitement partagé en lui injectant la désignation existante
        $this->enregistrerOuMettreAJour($request, $designation);
    ...
    /**
     * 🧠 FACTORISATION : Logique unique partagée pour le Store et l'Update.
     */
    private function enregistrerOuMettreAJour(Request $request, ?Designation $designation = null)
    {
        ...
        if (!is_null($valeurOrigine) && $valeurOrigine !== '' && $valeurOrigine !== 'null') {
            try {
                ...
                abort(back()->withErrors(['date_debut' => 'Le format de la date est invalide.']));
            }
        }

        // 2. CONFIGURATION DES RÈGLES DE VALIDATION
        $dateRules = ['required', 'date'];
        if (!is_null($dateDebutFormatee)) {
            $uniqueRule = Rule::unique('designations', 'date_debut');

            if ($designation) {
                $uniqueRule->ignore($designation->id);
            }

            $dateRules[] = $uniqueRule->where(function ($query) use ($request) {
        ...
        // 3. VÉRIFICATION DE LA GRILLE (Au moins un membre de choisi)
        ...
        foreach ($allDesignations as $labId => $jours) {
            foreach ($jours as $jourNom => $requis) {
                foreach ($requis as $requisConfigId => $membreId) { // 👈 Ex: 50 => 1
                    if (!empty($membreId)) {
        ...
        // 5. CRÉATION OU MISE À JOUR DE L'ENTITÉ PARENTE
        $dataParent = [
        ...
        if ($designation) {
            $designation->update($dataParent);
            $designation->items()->delete(); // On vide les anciens enfants avant de réinsérer
        } else {
            $dataParent['createur_id'] = $request->user()->id;
            $designation = Designation::create($dataParent);
        }

        // 6. ENREGISTREMENT DES ITEMS DE LA GRILLE CORRIGÉE 🚀
        // 6. ENREGISTREMENT DES ITEMS DE LA GRILLE CORRIGÉE 🚀
        $joursAjouter = [
            'vendredi'  => 0,
            'samedi'    => 1,
        ...
        foreach ($allDesignations as $labId => $jours) {
            foreach ($jours as $jourNom => $requis) {
                foreach ($requis as $requisConfigId => $membreId) {

                    if (!empty($membreId)) {
                        // On réinitialise à la date_debut pour chaque ligne
                        $dateEffective = Carbon::parse($dateDebutFormatee);

                        // 1. 🔍 Récupérer directement la configuration associée à cette case
                        $configRequis = LaboratoireConfig::find($requisConfigId);

                        if ($configRequis) {
                            // 2. 🎯 Si c'est une configuration de type "calendrier" (jours normaux de la semaine)
                            if ($configRequis->type_config === 'calendrier') {
                                $jourNomMinuscule = strtolower($jourNom);
                                if (array_key_exists($jourNomMinuscule, $joursAjouter)) {
                                    $dateEffective->addDays($joursAjouter[$jourNomMinuscule]);
                                }
                            }
                            // 3. 🎯 Si c'est une configuration de type "fixe" (vos Remplaçants)
                            elseif ($configRequis->type_config === 'fixe') {
                                // La dateEffective reste branchée sur $dateDebutFormatee par défaut.
                                // Vous n'avez pas besoin de calculer de décalage de jours ici.
                            }
                        }
                        ...
                        DesignationItem::create([
                            ...
                            'laboratoire_config_id' => $requisConfigId,
                            'membre_id'             => $membreId,
        ...
        // dd($allDesignations); // Debug : vérifier la structure de $allDesignations
        ...
        //             if (!empty($membreId)) {
        //                 // 🔥 IMPORTANT : On réinitialise à la date_debut pour chaque ligne
        //                 $dateEffective = Carbon::parse($dateDebutFormatee);

        //                 // Récupération de la configuration du labo
        //                 $configJour = LaboratoireConfig::where('jour_label', $jourNom)
        //                     ->where('laboratoire_id', $labId)
        //                     ->first();

        //                 // Si la config n'est pas fixe, on calcule le décalage de jours réels
        ...
        //                     'designation_id' => $designation->id,
        //                     'laboratoire_id' => $labId,
        //                     'laboratoire_config_id' => $requisConfigId, // 👈 C'est bien l'ID requis (ex: 50)
        //                     'membre_id' => $membreId,       // 👈 C'est l'ID du membre (ex: 1)
        //                     'date_effective' => $dateEffective->format('Y-m-d'),
        //                 ]);
        //             }
        //         }
    ...
    // private function enregistrerOuMettreAJour(Request $request, ?Designation $designation = null)
    // {
    //     dd($request->all()); // Debug : vérifier les données entrantes
    ...
    //     // 2. CONFIGURATION DES RÈGLES DE VALIDATION (Gestion dynamique de l'Ignore)
    //     $dateRules = ['required', 'date'];
    //     if (!is_null($dateDebutFormatee)) {
    //         $uniqueRule = Rule::unique('designations', 'date_debut');

    //         // 🔥 Si on est en mode UPDATE, on ignore l'ID en cours
    //         if ($designation) {
    //             $uniqueRule->ignore($designation->id);
    //         }
    ...
    //     // 3. VÉRIFICATION DE LA GRILLE (Au moins un membre coché)
    //     $hasAtLeastOneMember = false;
    //     $allDesignations = $request->input('all_designations', []);
    ...
    //         // Mode Update
    //         $designation->update($dataParent);
    //         // On nettoie les anciens enfants avant la réinsertion
    ...
    //     // 6. ENREGISTREMENT DES ITEMS DE LA GRILLE
    //     $joursAjouter = [
    //         'vendredi' => 0,
    //         'samedi' => 1,
    //         'dimanche' => 2,
    ...
    public function telechargerRapport($id)
    {
        ...
        /* TO DO : Sécurité d'accès à la désignation pour l'utilisateur connecté */

        // 1. Sécurité d'accès (Policy) 
        // $this->authorize('interagir', $designation);
