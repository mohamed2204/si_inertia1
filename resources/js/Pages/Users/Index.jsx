...
export default function Index({ all_groups, all_sous_depts = [], can_create }) {
    ...
    const loadUsers = async () => {
        ...
        try {
            ...
            toast.current.show({ severity: 'error', summary: 'Erreur', detail: 'Impossible de charger les utilisateurs' });
        } finally {
            setLoading(false);
        }
    };
    ...
    const saveUser = async () => {
        ...
        try {
            if (selectedUser) {
                ...
                toast.current.show({ severity: 'success', summary: 'Succès', detail: 'Utilisateur mis à jour' });
            } else {
            ...
            toast.current.show({ severity: 'error', summary: 'Échec', detail: msg });
        }
    };
    ...
    const deleteUser = async () => {
        try {
            ...
            toast.current.show({ severity: 'error', summary: 'Erreur', detail: 'Action impossible' });
        }
    };
    ...
    const actionBodyTemplate = (rowData) => {
        return (
            <div className="flex gap-2">
                {rowData.can_edit && (
                    <Button icon="pi pi-pencil" rounded severity="warning" size="small" onClick={() => editUser(rowData)} />
                )}
                {rowData.can_delete && (
                    <Button icon="pi pi-trash" rounded severity="danger" size="small" onClick={() => confirmDeleteUser(rowData)} />
                )}
            </div>
        );
    };
    ...
    const dialogFooter = (
        <div className="flex justify-end gap-2">
            <Button label="Annuler" icon="pi pi-times" outlined onClick={() => setUserDialog(false)} />
    ...
    const deleteDialogFooter = (
        <div className="flex justify-end gap-2">
            <Button label="Non" icon="pi pi-times" outlined onClick={() => setDeleteDialog(false)} />
            <Button label="Oui, Supprimer" icon="pi pi-check" severity="danger" onClick={deleteUser} />
        </div>
    );
    ...
    return (
        <Layout>
            <div className="card p-4 shadow-1 surface-card border-round-sm">
                <Toast ref={toast} />
                <h2 className="text-xl font-bold mb-4 text-slate-800">Gestion des Comptes Utilisateurs</h2>
                ...
                <DataTable
                    ...
                    onPage={(e) => setParams(prev => ({ ...prev, page: e.page + 1, per_page: e.rows }))}
                    onSort={(e) => setParams(prev => ({ ...prev, sort_by: e.sortField, sort_dir: e.sortOrder === 1 ? 'asc' : 'desc' }))}
                ...
                <Dialog visible={deleteDialog} style={{ width: '450px' }} header="Confirmer la suppression" modal footer={deleteDialogFooter} onHide={() => setDeleteDialog(false)}>
                    <div className="flex align-items-center gap-3">
                        <i className="pi pi-exclamation-triangle text-red-500 text-3xl" />
                        {selectedUser && (
                            <span>Êtes-vous sûr de vouloir supprimer définitivement le compte de <b>{selectedUser.name}</b> ?</span>
                        )}
                    </div>
                </Dialog>
            </div>
