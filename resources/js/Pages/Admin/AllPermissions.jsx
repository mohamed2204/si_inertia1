...
const AllPermissions = ({
    ...
    const checkModulePermission = (groupId, moduleCode, action) => {
        return modulePermissions.some(
            (p) =>
                p.group_id === groupId &&
                p.module_type === moduleCode &&
                p.type_action === action,
        );
    };
    ...
    const handleModuleToggle = (groupId, moduleCode, action) => {
        ...
        router.post(
            route("admin.permissions.modules.toggle"),
            {
                group_id: groupId,
    ...
    const handleTerrainToggle = (
        ...
        router.post(
            route("admin.permissions.terrain.toggle"),
            {
                user_id: userId,
            ...
            },
            {
                ...
                onSuccess: () => {
                    ...
                    Swal.fire({
                        icon: "success",
                        title: "Droit terrain mis à jour",
                        toast: true,
                        position: "top-end",
                        showConfirmButton: false,
                        timer: 900,
    ...
    const sousDepartementsFiltres = useMemo(() => {
        ...
        return sousDepartements.filter(
            (sd) => sd.nom && sd.nom.startsWith(`${selectedDepartement} - `),
        );
    ...
    const groupeOptions = useMemo(() => {
        ...
        return [{ label: "Tous les groupes", value: "ALL" }, ...tousLesGroupes];
    }, [utilisateurs]);
            ...
            <div className="p-4">
                <div className="mb-4">
                    ...
                    <p className="text-500 text-sm m-0 mt-1">
                        Configurez globalement les accès aux fonctionnalités de
                        l'application et restreignez précisément les données par
                        terrain.
                    </p>
                </div>

                <div className="surface-card shadow-2 border-round-xl p-2">
                    <TabView
                    ...
                    >
                        ...
                        >
                                ...
                                <table className="w-full text-left border-collapse custom-matrix-table">
                                    <thead>
                                        <tr className="surface-100 border-bottom-2 surface-border">
                                            <th
                                                className="p-3 text-700 font-bold text-sm"
                                                style={{ width: "30%" }}
                                            >
                                                Modules Applicatifs
                                            </th>
                                            {groupes.map((g) => (
                                                <th
                                                    key={g.id}
                                                    className="p-3 text-700 font-bold text-sm text-center"
                                                >
                                                    {g.name}
                                                </th>
                                            ))}
                                    ...
                                    <tbody>
                                        {modules.map((mod) => (
                                            <tr
                                                key={mod.code}
                                                className="hover:surface-50 border-bottom-1 surface-border"
                                            >
                                                <td className="p-3 font-semibold text-sm text-800">
                                                    {mod.name}
                                                </td>
                                                {groupes.map((g) => {
                                                    const isRead =
                                                        ...
                                                        >
                                                            <div className="flex justify-content-center align-items-center gap-3">
                                                                <div className="flex align-items-center gap-1">
                                                                    ...
                                                                    />
                                                                    <label
                                                                        htmlFor={`${currentKey}-r`}
                                                                        className="text-xs text-500 cursor-pointer font-bold"
                                                                    >
                                                                        L
                                                                    </label>
                                                                </div>
                            ...
                            <div className="text-xs text-500 mt-3 p-3 bg-blue-50 border-round border-left-3 border-blue-500">
                                📌 <strong>Légende Modules :</strong>{" "}
                                <strong>L</strong> = Voir le menu |{" "}
                                <strong>É</strong> = Ajouter/Modifier |{" "}
                                <strong>S</strong> = Supprimer. Les
                                modifications s'appliquent à tous les membres du
                                groupe.
                            </div>
                        </TabPanel>
                        ...
                        {/* ================================================================= */}
                        <TabPanel
                            header="2. Habilitations Terrains (CRUD)"
                        ...
                        >
                            ...
                            <div className="flex justify-content-end gap-3 mb-3 p-2 flex-wrap">
                                ...
                                <div
                                    className="flex align-items-center gap-2 bg-gray-50 p-2 border-round border-1 surface-border"
                                ...
                                <table className="w-full text-left border-collapse custom-matrix-table">
                                    <thead>
                                        <tr className="surface-100 border-bottom-2 surface-border">
                                            ...
                                            {sousDepartementsFiltres.map(
                                                (sd) => (
                                                    <th
                                                        key={sd.id}
                                                        className="p-3 text-700 font-bold text-sm text-center border-left-1 surface-border"
                                                        style={{
                                                            minWidth: "180px",
                                                        }}
                                                    >
                                    ...
                                    <tbody>
                                        {utilisateursFiltres.length === 0 ? (
                                            <tr>
                                                <td
                                                    colSpan={
                                                        sousDepartementsFiltres.length +
                                                        1
                                                    }
                                                    className="text-center p-5 text-500 italic"
                                                >
                                            ...
                                            utilisateursFiltres.map((user) => (
                                                ...
                                                >
                                                    <td className="p-3">
                                                        <div className="font-semibold text-sm text-800">
                                                            {user.name}
                                                        </div>
                                                        ...
                                                        <div className="flex flex-wrap gap-1">
                                                            {(
                                                                ...
                                                                <span
                                                                    key={g.id}
                                                                    className="bg-blue-50 text-blue-600 text-xs px-2 py-0.5 border-round font-medium"
                                                                >
                                                                    {g.name}
                                                                </span>
                                                            ))}
                                                    ...
                                                    {sousDepartementsFiltres.map(
                                                        (sd) => {
                                                            ...
                                                            return (
                                                                <td
                                                                    key={sd.id}
                                                                    className="p-3 border-left-1 surface-border bg-gray-50/50 text-center"
                                                                >
                                                                    <div className="flex justify-content-center align-items-center gap-3">
                            ...
                            <div className="text-xs text-500 mt-3 p-3 bg-green-50 border-round border-left-3 border-green-500">
                                💡 <strong>Périmètre Terrains :</strong> C =
                                Créer | R = Lire | U = Modifier | D = Supprimer.
                                Si un utilisateur n'a aucune case cochée sur un
                                sous-département, il ne pourra ni voir ni
